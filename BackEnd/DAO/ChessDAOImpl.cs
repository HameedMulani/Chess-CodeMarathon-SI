...
namespace ChessApiDemo.DAO
{
    public class ChessDAOImpl : IChessDAO
    {
        ...
        public async Task<List<Player>> GetPlayerByCountry(string country,string column)
        {
            ...
            string query = $"select * from hm.players where country = @country order by {column}";
        ...
        public async Task<int> AddMatch(Match m)
        {
            ...
            string insertQuery = @$"INSERT INTO hm.Matches (player1_id, player2_id, match_date, match_level, winner_id) VALUES  ('{m.Player1Id}', '{m.Player2Id}','{m.MatchDate}','{m.MatchLevel}', '{m.WinnerId}');";
